...
function createWorker(name, script, timeout) {
  ...
  w.boot = function() {
    w.process = spawn('python', ['-u', path.join(__dirname, 'shifu_ocr', script)], { cwd: __dirname, stdio: ['pipe', 'pipe', 'pipe'], env: { ...process.env, PYTHONUNBUFFERED: '1' } });
...
const brain = createWorker('Brain', 'shifu_brain.py', 60000);
brain.boot();
...
// ═══ HEARTBEAT PUMP — 1 BPS, busy-aware ═══
let _pumpBusy = false;
setTimeout(() => {
  setInterval(() => {
    if (brain.ready && !_pumpBusy) {
      _pumpBusy = true;
      brain.command({ cmd: 'ping' }).then(() => { _pumpBusy = false; }).catch(() => { _pumpBusy = false; });
    }
  }, 1000);
}, 10000);
...
const server = http.createServer(async (req, res) => {
  ...
  if (path === '/api/mind/replay' && req.method === 'POST') {
    const { k } = await parseBody(req);
    return jsonResponse(res, await mindCommand({ cmd: 'replay', k: k || 20 }));
  }
  ...
  // ═══ DATASET PROXY — fetch from HuggingFace server-side (no CORS) ═══
  if (path === '/api/dataset/fetch' && req.method === 'POST') {
    const { name } = await parseBody(req);
    try {
      const https = require('https');
      const dsName = (name || '').replace(/^hf:/, '').trim();
      // Paginate — HF API caps at 100 per request. Fetch up to 1000.
      const allSentences = [];
      let textCol = null;
      let offset = 0;
      const perPage = 100;
      const maxPages = 10; // 10 × 100 = 1000 sentences max

      for (let page = 0; page < maxPages; page++) {
        const hfUrl = `https://datasets-server.huggingface.co/rows?dataset=${encodeURIComponent(dsName)}&config=default&split=train&offset=${offset}&length=${perPage}`;
        const data = await new Promise((resolve, reject) => {
          https.get(hfUrl, { headers: { 'User-Agent': 'Shifu/1.0' } }, (resp) => {
            if (resp.statusCode >= 300 && resp.statusCode < 400 && resp.headers.location) {
              https.get(resp.headers.location, { headers: { 'User-Agent': 'Shifu/1.0' } }, (resp2) => {
                let body = '';
                resp2.on('data', (chunk) => body += chunk);
                resp2.on('end', () => { try { resolve(JSON.parse(body)); } catch(e) { resolve({}); } });
              }).on('error', () => resolve({}));
              return;
            }
            let body = '';
        ...
        // Detect text column on first page
        if (!textCol) {
          const firstRow = data.rows[0].row;
          textCol = Object.keys(firstRow).find(k => typeof firstRow[k] === 'string' && firstRow[k].length > 10) || Object.keys(firstRow)[0];
        }

        for (const r of data.rows) {
          const text = r.row[textCol];
          if (text && text.length > 10) allSentences.push(text);
        }

        offset += perPage;
        if (data.rows.length < perPage) break; // No more data
      }

      if (allSentences.length > 0) {
        return jsonResponse(res, { ok: true, sentences: allSentences, column: textCol, total: allSentences.length });
      }
      return jsonResponse(res, { ok: false, error: 'No rows found. Check dataset name.' });
    } catch (e) {
...
function detect(t){
  ...
  if(t.startsWith('/dataset '))return{i:'dataset',p:t.slice(9).trim()};
  ...
  try{
    if(d.i==='correct'){
      ...
      if(r.ok){
        ...
        // Pick FIRST long focus word and describe it — ONE call, not 6 sequential
        const fw=(r.focus||[]).find(w=>w.length>=4);
      ...
      // Single heartbeat call after consolidation (brain does 5 internally via thalamus)
      const hr=await api('/api/mind/heartbeat',{});
      h='<div><b>Consolidated:</b> '+r.routed+' routed, '+r.identities+' identities, '+r.pruned+' pruned</div>';
      h+='<div class="trace"><span class="lbl">myelinated</span> '+(hr.myelinated_new||0)+' new &middot; <span class="lbl">shortcuts</span> '+(hr.shortcuts||0)+' saltatory paths</div>';
      ...
      // Download via server-side proxy (avoids CORS) then feed in waves
      h='<div>Downloading <b>'+d.p+'</b>...</div>';
      ld.querySelector('.bubble').innerHTML=h;
      try{
        const dsResp=await api('/api/dataset/fetch',{name:d.p});
        if(dsResp.ok&&dsResp.sentences&&dsResp.sentences.length>0){
          const sentences=dsResp.sentences;
          h='<div>Downloaded <b>'+sentences.length+'</b> sentences from <b>'+d.p+'</b></div>';
          ld.querySelector('.bubble').innerHTML=h;
          let fed=0,wave=0;
          for(let pos=0;pos<sentences.length;){
            wave++;
            const stats=await api('/api/mind/stats');
            const vocab=stats.vocabulary||0;
            const biteSize=vocab<50?5:vocab<500?20:vocab<5000?100:500;
            const stage=vocab<50?'milk':vocab<500?'puree':vocab<5000?'solid':'adult';
            const bite=sentences.slice(pos,pos+biteSize);
            const fr=await api('/api/mind/feed-batch',{texts:bite});
            fed+=(fr.accepted||0);
            const wb=document.createElement('div');wb.className='msg shifu';
            wb.innerHTML='<div class="bubble" style="font-size:11px;color:var(--dim);padding:8px 12px;background:rgba(99,102,241,0.03)"><span style="color:var(--accent);font-weight:600;font-size:9px;text-transform:uppercase;letter-spacing:1px">'+stage+'</span> wave '+wave+': '+(fr.accepted||0)+' eaten, '+(fr.replayed||0)+' digested</div>';
            chat.appendChild(wb);chat.scrollTop=chat.scrollHeight;
            pos+=biteSize;
          }
          h='<div style="color:var(--green);font-weight:600">Fed '+fed+' sentences from '+d.p+' in '+wave+' waves</div>';
        }else{
          h='<div style="color:var(--red)">'+(dsResp.error||'No data found')+'. Try: /dataset agentlans/high-quality-english-sentences</div>';
        }
      }catch(e){h='<div style="color:var(--red)">Error: '+e.message+'</div>'}
    }
...
async function handleFiles(event) {
  ...
  // PHASE 2: Feed in WAVES — the baby eats what it can digest
  // Don't dump the whole elephant. Feed a wave, let it digest,
  // check if it's ready, feed more. Like a real meal.
  if (allSentences.length > 0) {
    const t0 = Date.now();
    let wave = 0;
    let pos = 0;
    while (pos < allSentences.length) {
      wave++;
      // Ask the brain how much it can eat (developmental stage)
      const stats = await api('/api/mind/stats');
      const vocab = (stats.ok ? stats.vocabulary : 0) || 0;
      let biteSize;
      if (vocab < 50) biteSize = 5;        // Milk
      else if (vocab < 500) biteSize = 20;  // Puree
      else if (vocab < 5000) biteSize = 100; // Solid
      else biteSize = 500;                   // Adult

      const bite = allSentences.slice(pos, pos + biteSize);
      const pct = 55 + Math.round((pos / allSentences.length) * 35);
      const stage = vocab < 50 ? 'milk' : vocab < 500 ? 'puree' : vocab < 5000 ? 'solid' : 'adult';
      showBar(pct, 'Wave ' + wave + ' (' + stage + '): feeding ' + bite.length + '/' + allSentences.length + ' sentences...');

      const fr = await api('/api/mind/feed-batch', { texts: bite });
      totalFed += (fr.accepted || 0);

      // Show progress in chat
      const waveBubble = document.createElement('div');
      waveBubble.className = 'msg shifu';
      waveBubble.innerHTML = '<div class="bubble" style="font-size:11px;color:var(--dim);padding:8px 12px;background:rgba(99,102,241,0.03)"><span style="color:var(--accent);font-weight:600;font-size:9px;text-transform:uppercase;letter-spacing:1px">'+stage+'</span> wave ' + wave + ': ' + (fr.accepted||0) + ' eaten, ' + (fr.replayed||0) + ' digested, ' + (fr.neurons_born||0) + ' neurons born</div>';
      chat.appendChild(waveBubble);
      chat.scrollTop = chat.scrollHeight;

      pos += biteSize;
    }
    const elapsed = ((Date.now() - t0) / 1000).toFixed(1);
    const rate = Math.round(allSentences.length / Math.max((Date.now() - t0) / 1000, 0.1));
    results += '<div class="trace"><span class="lbl">fed</span> ' + totalFed + '/' + allSentences.length + ' in ' + wave + ' waves, ' + elapsed + 's (' + rate + ' sent/sec)</div>';
  }
...
let _statsTimer=null,_lastBabble=0,_lastCry=0;
function updateStats(){
  if(_statsTimer)return;
  _statsTimer=setTimeout(async()=>{
    _statsTimer=null;
    try{
      // ONLY stats — lightweight, no side effects
      const m=await api('/api/mind/stats');
...
// Separate slow poll for autonomous actions — runs less often, doesn't block stats
let _autoTimer=null;
function autoTick(){
  ...
  _autoTimer=setTimeout(async()=>{
    _autoTimer=null;
    try{
      const a=await api('/api/mind/idle',{});
      ...
      if(a.ok){
        ...
        pLabel.textContent=a.did==='rest'?'resting':(a.did+': '+(a.result||''));
        if(a.did!=='rest')pLabel.className='learning-label practicing';
        // BABBLE: throttle to at most once per 30 seconds
        const now=Date.now();
        if(a.did!=='rest'&&now-_lastBabble>30000){
          _lastBabble=now;
      ...
      // CRY removed from auto — too heavy. Use /cry command manually.
    }catch{}
  },500);
}
setInterval(updateStats, 30000);  // Stats every 30s
setInterval(autoTick, 30000);    // Babble every 30s
updateStats();
</script>
</body>
</html>`;
