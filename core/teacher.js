...
class Teacher {
  ...
  diagnose() {
    ...
    // Uses pressureOf() which reads node.prev (O(1)), not full scan
    const starved = [];
    for (const [w, n] of Object.entries(this.engine.nodes)) {
  ...
  lesson(text, domain = "general") {
    ...
    // Decay noise immediately (skip if engine is tiny)
    ...
    // Sample pressure on words from this text only (not full graph)
  ...
  // ─── Spontaneous: the engine teaches itself ─────────────────────
  // Reads pressure, generates sentences from its own grooves,
  // feeds them back, consolidates. No external input needed.
  // Call this on a timer or after each feed.
  spontaneous() {
    const eng = this.engine;
    if (Object.keys(eng.nodes).length < 10) return { action: "wait", reason: "too few words" };

    const diag = this.diagnose();
    const actions = [];

    // 1. Generate sentences from starved words by walking grooves
    for (const s of diag.starved.slice(0, 3)) {
      const generated = this._walkFrom(s.word, 8);
      if (generated) {
        eng.feed(generated);
        actions.push({ type: "reinforce", word: s.word, sentence: generated });
      }
    }

    // 2. Generate sentences from underexposed words
    for (const u of diag.underexposed.slice(0, 3)) {
      const generated = this._walkFrom(u.word, 8);
      if (generated) {
        eng.feed(generated);
        actions.push({ type: "reinforce", word: u.word, sentence: generated });
      }
    }

    // 3. Cross-pollinate: connect high-pressure surpluses with vacuums
    // Walk from a surplus toward a vacuum through the graph
    if (diag.surpluses.length > 0 && diag.vacuums.length > 0) {
      const src = diag.surpluses[0].word;
      const dst = diag.vacuums[0].word;
      const bridge = this._walkBetween(src, dst, 10);
      if (bridge) {
        eng.feed(bridge);
        actions.push({ type: "bridge", from: src, to: dst, sentence: bridge });
      }
    }

    // 4. Contrastive drill on a random structured word
    const structured = Object.keys(eng.nodes).filter(w => {
      const d = eng.depth(w);
      return d.level === "structured" || d.level === "deep";
    });
    let drillResult = null;
    if (structured.length >= 2) {
      const a = structured[Math.floor(Math.random() * structured.length)];
      const sentence = this._walkFrom(a, 6);
      if (sentence) drillResult = this.drill(sentence);
    }

    // 5. Decay noise
    const decayed = eng.decay(1);
  ...
  // Walk the nx chain from a starting word to generate a sentence
  _walkFrom(start, maxLen) {
    const eng = this.engine;
    const node = eng.nodes[start];
    if (!node) return null;
    ...
    for (let i = 0; i < maxLen - 1; i++) {
      const nd = eng.nodes[current];
      if (!nd || !nd.next || Object.keys(nd.next).length === 0) break;

      // Pick next word weighted by frequency in nx table
      const entries = Object.entries(nd.next);
      const total = entries.reduce((s, [, w]) => s + w, 0);
      let r = Math.random() * total;
      let picked = entries[0][0];
      for (const [word, weight] of entries) {
        r -= weight;
        if (r <= 0) { picked = word; break; }
      }

      // Avoid loops
      if (words.includes(picked) && words.length > 2) break;
      words.push(picked);
      current = picked;
    }
  ...
  // Try to walk from src toward dst through the graph
  _walkBetween(src, dst, maxLen) {
    const eng = this.engine;
    if (!eng.nodes[src] || !eng.nodes[dst]) return null;
    ...
    for (let i = 0; i < maxLen - 1; i++) {
      ...
      // If dst is directly reachable, go there
      if (nd.next[dst]) {
        words.push(dst);
        break;
      }

      // Pick the next word that's closest to dst (shares most neighbors)
      const dstNb = new Set(Object.keys(eng.nodes[dst]?.neighbors || {}));
      let bestWord = null, bestScore = -1;

      for (const [candidate, weight] of Object.entries(nd.next)) {
        if (words.includes(candidate)) continue;
        const cNb = Object.keys(eng.nodes[candidate]?.neighbors || {});
        const overlap = cNb.filter(w => dstNb.has(w)).length;
        const score = overlap + weight * 0.1;
        if (score > bestScore) { bestScore = score; bestWord = candidate; }
      }
    ...
    // Only useful if we got close to or reached dst
    return words.length >= 3 ? words.join(" ") : null;
  }
