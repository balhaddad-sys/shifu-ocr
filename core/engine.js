...
feed(raw){
  ...
  for(let i=0;i<ws.length;i++){
    ...
    for(let j=Math.max(0,i-win);j<Math.min(len,i+win+1);j++){
      ...
      if(existing){nd.neighbors[nb]=existing+1;}
      else{
        if(hasStructure&&this.nodes[nb]){
          ...
          if(phase<0.05){nd.neighbors[nb]=plasticity*0.5;const weakest=Object.entries(nd.neighbors).filter(([k])=>k!==nb).sort((a,b)=>a[1]-b[1]).slice(0,2);for(const[k]of weakest){nd.neighbors[k]*=0.95;if(nd.neighbors[k]<0.5)delete nd.neighbors[k];}}
          else{nd.neighbors[nb]=Math.max(plasticity,0.5);}
        }else{nd.neighbors[nb]=nd.freq<=5?1:Math.max(plasticity*2,0.5);}
      }
    }
...
compare(a,b){
  ...
  if(na.freq>=3&&nb.freq>=3){sig.posSim=1-Math.min(Math.abs(na.pos-nb.pos)*2,1);const sw=Math.min((1-na.posV)*(1-nb.posV)*4,1);wt.pos=sw*0.15;}
