...
feed(raw){
  ...
  for(let i=0;i<ws.length;i++){
    ...
    if(nd.freq===1){nd.pos=rp;nd.posV=0.25;}
    else{const delta=rp-nd.pos;nd.pos+=delta*plasticity;nd.posV+=(delta*delta-nd.posV)*plasticity;}
    ...
    if(!nd.frozen){
      ...
      for(let j=Math.max(0,i-win);j<Math.min(len,i+win+1);j++){
        ...
        if(existing){nd.neighbors[nb]=Math.min(existing+1,nd.freq/3);}
        else{
          let add=nd.freq<=5?1:Math.max(plasticity*2,0.5);
          if(hasStructure&&this.nodes[nb]){
            const nbNb=this.nodes[nb].neighbors||{};
            const touch=(prevW&&nbNb[prevW]?1:0)+(nextW&&nbNb[nextW]?1:0);
...
depth(w){
  ...
  const nb=n.nbrCount;const sq=Object.keys(n.next).length+Object.keys(n.prev).length;
...
compare(a,b){const al=a.toLowerCase(),bl=b.toLowerCase(),na=this.nodes[al],nb=this.nodes[bl],sig={},wt={};sig.editSim=1-editDistance(al,bl)/Math.max(al.length,bl.length,1);sig.bigramSim=sharedBigrams(al,bl);sig.ocrSim=1-ocrDistance(al,bl)/Math.max(al.length,bl.length,1);wt.char=0.05;if(!na||!nb)return{similarity:sig.editSim*0.3+sig.bigramSim*0.3+sig.ocrSim*0.4,signals:sig,weights:wt,totalWeight:0.05,depth:"surface"};const nbA=Object.keys(na.neighbors),nbBs=new Set(Object.keys(nb.neighbors)),shared=nbA.filter(x=>nbBs.has(x)),union=new Set([...nbA,...nbBs]);if(union.size>0){const rw=w=>1/Math.max(Math.log2((this.nodes[w]?.freq||0)+1),1);const sw=shared.reduce((s,w)=>s+rw(w),0),uw=[...union].reduce((s,w)=>s+rw(w),0);sig.neighborOverlap=uw>0?sw/uw:0;wt.neighbor=Math.min(union.size/10,0.35);}const totA=Object.values(na.next).reduce((s,v)=>s+v,0),totB=Object.values(nb.next).reduce((s,v)=>s+v,0);if(totA>0||totB>0){sig.expectsAB=totA?(na.next[bl]||0)/totA:0;sig.expectsBA=totB?(nb.next[al]||0)/totB:0;sig.directional=Math.abs(sig.expectsAB-sig.expectsBA);wt.seq=Math.min((totA+totB)/20,0.25);}const nx2AB=na.next2?.[bl]?Object.keys(na.next2[bl]).length:0,nx2BA=nb.next2?.[al]?Object.keys(nb.next2[al]).length:0;if(nx2AB>0||nx2BA>0){sig.trajectoryAB=Math.min(nx2AB/5,1);sig.trajectoryBA=Math.min(nx2BA/5,1);wt.traj=0.15;}if(na.freq>=3&&nb.freq>=3){sig.posSim=1-Math.min(Math.abs(na.pos-nb.pos)*2,1);const sw2=Math.min((1-na.posV)*(1-nb.posV)*4,1);wt.pos=sw2*0.15;}let indAB=0;for(const mid of Object.keys(na.next))if(this.nodes[mid]?.next[bl])indAB++;if(indAB>0){sig.indirectAB=Math.min(indAB/5,1);wt.indirect=Math.min(indAB/10,0.15);}let sim=0,tw=0;if(wt.char){sim+=(sig.editSim*0.3+sig.bigramSim*0.3+sig.ocrSim*0.4)*wt.char;tw+=wt.char;}if(wt.neighbor){sim+=sig.neighborOverlap*wt.neighbor;tw+=wt.neighbor;}if(wt.seq){sim+=Math.max(sig.expectsAB||0,sig.expectsBA||0)*wt.seq;tw+=wt.seq;}if(wt.traj){sim+=Math.max(sig.trajectoryAB||0,sig.trajectoryBA||0)*wt.traj;tw+=wt.traj;}if(wt.pos){sim+=sig.posSim*wt.pos;tw+=wt.pos;}if(wt.indirect){sim+=sig.indirectAB*wt.indirect;tw+=wt.indirect;}if(tw>0)sim/=tw;const d=tw>0.5?"deep":tw>0.3?"structured":tw>0.15?"forming":"shallow";return{similarity:sim,signals:sig,weights:wt,totalWeight:tw,depth:d};}

affinity(a,b){const al=a.toLowerCase(),bl=b.toLowerCase(),na=this.nodes[al],nb=this.nodes[bl];if(!na||!nb)return{a:al,b:bl,mutual:0,known:false};const nbA=Object.keys(na.neighbors),nbBs=new Set(Object.keys(nb.neighbors)),shared=nbA.filter(x=>nbBs.has(x));const rw=w=>1/Math.max(Math.log2((this.nodes[w]?.freq||0)+1),1);const sw=shared.reduce((s,w)=>s+rw(w),0),uw=[...new Set([...nbA,...nbBs])].reduce((s,w)=>s+rw(w),0);const orbit=uw>0?sw/uw:0;const totA=Object.values(na.next||{}).reduce((s,v)=>s+v,0),totB=Object.values(nb.next||{}).reduce((s,v)=>s+v,0);const pullAB=totA?(na.next[bl]||0)/totA:0,pullBA=totB?(nb.next[al]||0)/totB:0;let indAB=0,indBA=0;for(const mid of Object.keys(na.next||{}))if(this.nodes[mid]?.next[bl])indAB++;for(const mid of Object.keys(nb.next||{}))if(this.nodes[mid]?.next[al])indBA++;indAB=Math.min(indAB/5,1);indBA=Math.min(indBA/5,1);const charSim=1-editDistance(al,bl)/Math.max(al.length,bl.length,1);const fwA=new Set(Object.keys(na.next||{})),fwB=new Set(Object.keys(nb.next||{})),fwU=new Set([...fwA,...fwB]);const expOvlp=fwU.size?[...fwA].filter(x=>fwB.has(x)).length/fwU.size:0;let posAlign=0;if(na.freq>=3&&nb.freq>=3)posAlign=1-Math.min(Math.abs(na.pos-nb.pos)*3,1);const afAB=charSim*0.05+orbit*0.35+pullAB*0.20+indAB*0.20+expOvlp*0.15+posAlign*0.05;const afBA=charSim*0.05+orbit*0.35+pullBA*0.20+indBA*0.20+expOvlp*0.15+posAlign*0.05;return{a:al,b:bl,orbit,pullAB,pullBA,indAB,indBA,charSim,expOvlp,posAlign,afAB,afBA,mutual:(afAB+afBA)/2,asym:Math.abs(afAB-afBA),known:true};}
...
scoreSentence(raw){
  ...
  for(let i=0;i<ws.length;i++){
    ...
    step.afGate=0;
    if(i>0&&node){const prevNode=this.nodes[ws[i-1]];const prevDeep=prevNode&&(prevNode.freq>=5&&prevNode.nbrCount>=5);
      if((wDeep||prevDeep)&&prevNode?.neighbors&&node.neighbors){const pN=Object.keys(prevNode.neighbors),wN=new Set(Object.keys(node.neighbors)),sh=pN.filter(x=>wN.has(x)).length,un=new Set([...pN,...wN]).size;step.afGate=un?sh/un:0;}}
...
compact(maxNbrs=30){let stripped=0;for(const[w,nd]of Object.entries(this.nodes)){if(nd.nbrCount<=maxNbrs)continue;const sorted=Object.entries(nd.neighbors).sort((a,b)=>b[1]-a[1]);const keep=new Set(sorted.slice(0,maxNbrs).map(([k])=>k));for(const[k]of sorted){if(!keep.has(k)){delete nd.neighbors[k];nd.nbrCount--;stripped++;}}}return{stripped};}
...
generate(startWord,goalWord,bridgeWord,maxLen=12){
  let w=startWord?startWord.toLowerCase():null;const goal=goalWord?goalWord.toLowerCase():null;const bridge=bridgeWord?bridgeWord.toLowerCase():null;
  ...
  for(let ii=0;ii<maxLen-1;ii++){const nd=this.nodes[w];if(!nd)break;const entries=Object.entries(nd.next).filter(([nw])=>!used.has(nw)||words.length>6);if(!entries.length)break;const seqTotal=entries.reduce((s,[,c])=>s+c,0);const fieldMax=Math.max(1,...field.values());const progress=words.length/maxLen;const target=(!bridgeReached&&bridge)?bridge:goal;const scored=entries.map(([nw,cnt])=>{const nNode=this.nodes[nw];const freq=nNode?.freq||1;const seq=cnt/seqTotal;const fld=(field.get(nw)||0)/fieldMax*(1/Math.max(Math.log2(freq+1),0.5));const mass=nNode&&nNode.nbrCount>=5&&freq>=5?1.2:freq>=3?0.8:0.4;let posfit=1;if(nNode&&nNode.freq>=3&&nNode.posV<0.2){posfit=1-Math.abs(nNode.pos-progress)*0.8;}let pull=0;if(target&&nNode?.next){if(nw===target)pull=3;else if(nNode.next?.[target])pull=1.5;else if(nNode.neighbors?.[target])pull=0.5;}return{w:nw,score:(seq*0.25+fld*0.25+pull*0.20+posfit*0.15)*mass+seq*0.15};});const totalScore=scored.reduce((s,x)=>s+Math.max(x.score,0.01),0);let r=Math.random()*totalScore,pick=scored[0].w;for(const x of scored){r-=Math.max(x.score,0.01);if(r<=0){pick=x.w;break;}}words.push(pick);used.add(pick);addToField(pick);w=pick;if(pick===bridge)bridgeReached=true;if(pick===goal)break;}
...
speak(topic,count=5,maxLen=12){
  ...
  const nbrs=Object.entries(tn.neighbors).filter(([w])=>{const nd=this.nodes[w];return nd&&nd.freq>=3&&nd.freq<ceiling&&nd.nbrCount>=3;}).sort((a,b)=>b[1]-a[1]).slice(0,20).map(([w])=>w);
  ...
  const allGoals=[...new Set([...nbrs,...seqNbrs])].filter(w=>w!==tl);const starts=[tl,...nbrs.slice(0,5)];
  ...
  for(let i=0;i<count;i++){const start=starts[i%starts.length];const goal=allGoals.length?allGoals[gi++%allGoals.length]:null;
