...
// v2.2.0: accommodation — the system adjusts until what it sees is sharpest
...
const CONFIG = {
  ...
  inhibition: { floor: 0.1, ceiling: 1.0 },
  correction: { strength: 0.6 },
  settle: { maxIter: 3, epsilon: 0.01, reactivateTop: 8, iterDecay: 0.5 },
  gate: { dampen: 0.4, boost: 1.5 },
};
...
class ShifuEngine {
  ...
  feed(raw){
    ...
    for(let i=0;i<ws.length;i++){
      ...
      this.co[w]??={};
      for(let j=Math.max(0,i-3);j<Math.min(ws.length,i+4);j++){if(j!==i){this.co[w][ws[j]]=(this.co[w][ws[j]]||0)+1;}}
      ...
      const nb=new Set();for(let j=Math.max(0,i-3);j<Math.min(ws.length,i+4);j++){if(j!==i)nb.add(ws[j]);}
  ...
  activate(w) {
    ...
    const field = new Map();
    const d1 = cfg.decay, d2 = d1 * d1;
    field.set(w, 1.0);
    const excite = (target, energy) => { const inh = this._inhibitionWeight(target); field.set(target, (field.get(target) || 0) + energy * inh); };
    const co = this.co[w];
  ...
  activateInContext(w, contextField = null) {
    ...
    const excite = (target, energy) => {
      ...
      const ctxNorm = ctxEnergy / maxCtx;
  ...
  settle(initialField) {
    ...
    for (let iter = 0; iter < cfg.maxIter; iter++) {
      ...
      const topNodes = [...currentField.entries()].sort((a, b) => b[1] - a[1]).slice(0, cfg.reactivateTop);
      for (const [node, energy] of topNodes) {
        if (!(node in this.wf)) continue;
  ...
  scoreSentence(raw) {
    ...
    for (let i = 0; i < ws.length; i++) {
      ...
      if (i > 0 && globalField.size > 0) { const fieldEnergy = globalField.get(w) || 0; const maxEnergy = Math.max(...globalField.values(), 1e-9); step.reinforcement = Math.min(fieldEnergy / maxEnergy, 1); } else { step.reinforcement = 0; }
      const wave = this.activateInContext(w, globalField);
      step.waveSize = wave.size;
      if (i > 0 && globalField.size > 0) { step.synchrony = fieldCosine(wave, globalField); } else { step.synchrony = 0; }
      if (i > 0) {
        const prev = ws[i - 1];
      ...
      steps.push(step);
      for (const [node, energy] of wave) { globalField.set(node, (globalField.get(node) || 0) + energy); }
    }
    ...
    for (let si = 0; si < steps.length; si++) {
      ...
      const priorSteps = steps.slice(1, si);
      ...
      const combinedGate = seqGate * (0.4 + consistency * 0.6);
    ...
    return { words: ws, steps, totalSurprise, meanSurprise, coherence: forwardCoherence, correctedCoherence, settledCoherence, field: settleResult.field, settled: settleResult.converged, settleIterations: settleResult.iterations };
  }
  ...
  accommodate(candidates, scorer) {
    if (!candidates || candidates.length === 0) return { best: null, scores: [] };
    if (candidates.length === 1) return { best: candidates[0], scores: [{ candidate: candidates[0], score: scorer(candidates[0]) }] };
    const scores = candidates.map(c => ({ candidate: c, score: scorer(c) }));
    scores.sort((a, b) => b.score - a.score);
    return { best: scores[0].candidate, bestScore: scores[0].score, margin: scores[0].score - scores[1].score, scores };
  }

  accommodateWord(words, position, candidates) {
    if (!candidates || candidates.length === 0 || position >= words.length) return { best: words[position], coherenceGain: 0, scores: [] };
    const baseline = this.scoreSentence(words.join(' '));
    const baselineCoh = baseline.correctedCoherence;
    const scores = candidates.map(candidate => {
      const testWords = [...words]; testWords[position] = candidate;
      const result = this.scoreSentence(testWords.join(' '));
      return { word: candidate, coherence: result.correctedCoherence, settled: result.settledCoherence, gain: result.correctedCoherence - baselineCoh };
    });
    scores.sort((a, b) => b.coherence - a.coherence);
    const best = scores[0];
    return { best: best.word, original: words[position], coherenceGain: best.gain, accepted: best.gain > -0.02, scores };
  }

  accommodateSentence(variants) {
