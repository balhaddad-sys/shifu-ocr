...
const{ShifuEngine,VERSION}=require("../core/engine");
...
async function persistAll(){
  for(const[id,s]of Object.entries(engines)){
    if(!s.dirty)continue;
    try{await store.save(id,s.eng.serialize());s.dirty=false;log("INFO",`Persisted: ${id}`,{vocab:s.eng.stats().vocabulary});}
...
app.post("/feed",withMetrics("feed",async(req,res,eid)=>{
  ...
  const eng=await getEngine(eid);const r=eng.feedText(req.body.text);markDirty(eid);
  res.json({engineId:eid,...r,stats:eng.stats()});
}));

app.post("/compare",withMetrics("compare",async(req,res,eid)=>{
  if(requireBody(res,req.body,["a","b"]))return;
  const{a,b,mode="meaning",mask}=req.body;
  res.json({engineId:eid,a,b,mode,...(await getEngine(eid)).compare(a,b,mode,mask||null)});
}));

app.post("/correct",withMetrics("correct",async(req,res,eid)=>{
  if(requireBody(res,req.body,["text"]))return;
  const{text,k=5,gate=true}=req.body;const eng=await getEngine(eid);
  const results=text.split(/\s+/).map(w=>{
    if(eng.wf[w.toLowerCase()])return{original:w,known:true};
    const r=eng.correct(w,k);
    if(gate&&r.reject)return{original:w,known:false,correction:null,gated:true,confidence:r.confidence};
    return{original:w,known:false,correction:r.candidates[0]?.word||null,confidence:r.confidence,lowConfidence:r.lowConfidence,candidates:r.candidates.slice(0,3).map(c=>({word:c.word,score:+c.routed.toFixed(4)}))};
  });
  log("INFO","correct_detail",{engineId:eid,words:results.length,gated:results.filter(r=>r.gated).length});
...
app.post("/explore",withMetrics("explore",async(req,res,eid)=>{
  ...
  res.json({engineId:eid,word:w,frequency:eng.wf[w]||0,form:Array.from(eng.formVec(w)),context:Array.from(eng.contextVec(w)),history:Array.from(eng.historyVec(w)),similar:eng.similar(w,10)});
}));
...
function smokeTest(){
  ...
  const v=eng.vec("doctor"),s=eng.scoreSentence("doctor treats patient"),r=eng.correct("seisure");
  const ok=v.length===60&&typeof s.coherence==="number"&&r.candidates.length>0;
  if(!ok){log("ERROR","Smoke test FAILED");process.exit(1);}
  log("INFO","Smoke test passed",{dim:v.length,coherence:+s.coherence.toFixed(3)});
}
